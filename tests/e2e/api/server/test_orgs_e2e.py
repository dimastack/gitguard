...
def test_org_lifecycle(gitea_client: GiteaHttpClient):
    ...
    assert result.ok() or result.status_code == 422, f"Org creation failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"List orgs failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"Get org failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"Edit org failed: {result.status_code} {result.text}"
    result = gitea_client.get_org(org_name)
    assert result.ok()
...
def test_create_duplicate_org(gitea_client: GiteaHttpClient):
    ...
    result = gitea_client.admin_create_org(owner_username=username, org_name=org_name)
    assert not result.ok(), f"Expected failure for duplicate org, got {result.status_code}"
