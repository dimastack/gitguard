...
def test_repo_lifecycle(gitea_client: GiteaHttpClient):
    ...
    assert result.ok() or result.status == 422, f"Precondition: user creation failed {result.status} {result.text}"

    # 1. create repository
    result = gitea_client.create_repo(repo_name, owner=username, private=False, description="Initial repo")
    assert result.ok(), f"Repo creation failed: {result.status} {result.text}"
    ...
    assert result.ok(), f"Get repo failed: {result.status} {result.text}"
    ...
    assert result.ok(), f"Edit repo failed: {result.status} {result.text}"
    result = gitea_client.get_repo(owner=username, repo=repo_name)
    assert result.ok()
    ...
    assert result.ok(), f"List user repos failed: {result.status} {result.text}"
    ...
    assert result.ok(), f"Delete repo failed: {result.status} {result.text}"
...
def test_create_duplicate_repo(gitea_client: GiteaHttpClient):
    ...
    assert result.ok() or result.status == 409

    # Second creation should fail (conflict)
    result = gitea_client.create_repo(repo_name, owner=username)
    assert not result.ok(), f"Expected conflict, got {result.status}"
