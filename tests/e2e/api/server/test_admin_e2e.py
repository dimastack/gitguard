...
def test_admin_create_edit_delete_user(gitea_client: GiteaHttpClient):
    ...
    assert result.ok() or result.status_code == 422, f"User creation failed: {result.status_code} {result.text}"

    # 2. verify in list_users
    result = gitea_client.list_users()
    assert result.ok(), f"List users failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"Edit user failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"Delete user failed: {result.status_code} {result.text}"
...
def test_admin_create_repo_for_user(gitea_client: GiteaHttpClient):
    ...
    assert result.ok() or result.status_code == 409, f"Repo creation failed: {result.status_code} {result.text}"
    ...
    assert result.ok(), f"Fetch repo failed: {result.status_code} {result.text}"
...
def test_admin_list_unadopted_repos(gitea_client: GiteaHttpClient):
    ...
    assert result.ok(), f"List unadopted repos failed: {result.status_code} {result.text}"
    data = result.json()
