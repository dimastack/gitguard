...
def test_git_remote_configuration(git_client, gitea_host, tmp_path, protocol):
    """
    Scenario:
    1. init repo
    2. add remote
    ...
    git_client.owner = "alice"
    git_client.repo = "remote-demo"
    git_client.workdir = str(tmp_path / "repo")
    ...
    # add remote
    result = git_client._run(["remote", "add", "origin", git_client.repo_url])
    assert result.ok(), f"Remote add failed: {result.stderr}"

    # verify remote
    result = git_client._run(["remote", "-v"])
    assert result.ok(), f"Remote -v failed: {result.stderr}"
    remotes = result.stdout.splitlines()
    assert any("origin" in r and git_client.repo_url in r for r in remotes), \
        f"Origin not found in remotes: {remotes}"

    # change remote URL
    new_url = git_client.repo_url.replace(git_client.repo, f"{git_client.repo}-alt")
    result = git_client._run(["remote", "set-url", "origin", new_url])
    ...
    # remove remote
    result = git_client._run(["remote", "remove", "origin"])
    ...
    # negative: fetch from removed remote
    result = git_client._run(["fetch", "origin"])
