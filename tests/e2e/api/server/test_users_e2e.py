...
def test_user_lifecycle(gitea_client: GiteaHttpClient):
    ...
    assert result.ok(), f"User creation failed: {result.status_code} {result.text}"

    # 2. fetch user
    result = gitea_client.get_user(username)
    assert result.ok(), f"Get user failed: {result.status_code} {result.text}"
    data = result.json()
    ...
    result = gitea_client.get_user(username)
    assert result.ok(), f"Re-fetch user after edit failed: {result.status_code} {result.text}"
...
def test_get_nonexistent_user(gitea_client: GiteaHttpClient):
    ...
    assert result.status_code == 404, f"Expected 404, got {result.status_code} {result.text}"
